        ...
        if multipathDevices != nil && len(multipathDevices) > 0 {
                for _, device := range multipathDevices {
                        ...
                        if device.IsUnhealthy {
                                log.Infof("Multipath device %s is unhealthy and is present on %s node", device.Name, nodeName)
                        } else {
                                log.Infof("Multipath device %s is healthy and is present on %s node", device.Name, nodeName)
                        }
                }
        } else {
