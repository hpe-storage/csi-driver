...
It is recommended to to treat this as separate edit than other backend volume properties|
...
|| cloneOf, virtualCopyOf, importVolAsClone| I/O restrictions |Source volumes used as part of `virtualCopyOf`, `cloneOf`,'importVolAsClone` options should not having any I/O activity taking place, when this snapshot/clone operation is done via CSI driver. This might lead to data consistency issues, due to the hidden snapshots taken by the array|None|